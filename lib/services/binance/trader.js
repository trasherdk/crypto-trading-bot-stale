...
/** closed orders recency */
const DAYS_TO_EVALUATE = 28;

const LUNAR_SPREAD_IMPACT = 0.05; // [0, 1) {0: no impact}
...
/**
 * SELL|BUY waves impact multiplier [0, n] where 1 is directly proportional to wave height
 * 
 * e.g. as price spread impact to determine whether a new order placing is a good option after previous trade
 * */
const WAVE_IMPACT_MULTIPLIER = 0.25;
...
export default class BinanceTrader {
        ...
        if (buyOpenOrder !== null) {
            ...
            if (this.closedOrders.length > 0) {
                ...
                if (lastBuyOrder !== null && lastSellOrder !== null) {
                    ...
                    const spread = this.getBuySpread() * (1 / WAVE_IMPACT_MULTIPLIER);
                    ...
                    if (delta1 > spread && delta2 > spread && delta3 > spread && delta4 < MAX_SPREAD) {
        ...
        if (sellOpenOrder !== null) {
            ...
            if (this.closedOrders.length > 0) {
                ...
                if (lastBuyOrder !== null && lastSellOrder !== null) {
                    ...
                    const spread = this.getSellSpread() * (1 / WAVE_IMPACT_MULTIPLIER);
    ...
    getBuyOrder = () => {
        ...
        /**
         * slow-down BUY when bullish
         * speed-up  BUY when bearish (by default)
        */
        let sellWaveImpact = 1;
        sellWaveImpact = this.getLastWaveHeight('sell') * WAVE_IMPACT_MULTIPLIER;
        if (sellWaveImpact > 1) {
            buyPrice = this.price.bid * (1 - this.getBuySpread() * sellWaveImpact);
            console.log(`[${ this.market }] [i] [>>] sell wave impact ${ sellWaveImpact }`);
        }
    ...
    getSellOrder = () => {
        ...
        /** 
         * slow-down SELL when bearish
         * speed-up  SELL when bullish (by default)
        */
        let buyWaveImpact = 1;
        buyWaveImpact = this.getLastWaveHeight('buy') * WAVE_IMPACT_MULTIPLIER;
        if (buyWaveImpact > 1) {
            sellPrice = this.price.ask * (1 + this.getSellSpread() * buyWaveImpact);
            console.log(`[${ this.market }] [i] [<<] buy wave impact ${ buyWaveImpact }`);
        }
