...
class Trader {
        ...
        if (this.isOrderOpen('buy')) {
            ...
            if (this.closedOrders.length > 0) {
                ...
                if (lastClosedOrder.side === 'sell') {
                    const delta = (lastClosedOrder.price - order.buyPrice) / order.buyPrice;
                    if (delta > this.buySpread) {
    ...
    printIterationInfo = () => {
        ...
        for (const order of this.openOrders) {
            console.log(`[${ this.market }] [i]`, order.side, order.amount, this.asset.symbol, '@', order.price, '=>', order.amount * order.price, this.base.symbol);
        }
    };
};
