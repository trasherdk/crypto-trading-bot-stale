...
class Trader {
    ...
    shouldBuy = (order) => {
        if (this.isOrderOpen('buy')) {
            console.log('[...] buy order is still open');
            return false;
        }

        if (order.totalToBeBought < MIN_ORDER_VOLUME) {
            console.log('[...] buy order cannot be created', this.round(order.totalToBeBought));
            return false;
        }
        ...
        if (order.buyPrice > sellOpenOrder.price) {
            console.log(`[...] buy price is higher than sell: ${order.buyPrice} > ${sellOpenOrder.price}`);
            return false;
        }
    ...
    shouldSell = (order) => {
        if (this.isOrderOpen('sell')) {
            console.log('[...] sell order is still open');
            return false;
        }

        if (order.totalToBeSold < MIN_ORDER_VOLUME) {
            console.log('[...] sell order cannot be created', this.round(order.totalToBeSold));
            return false;
        }
        ...
        // TODO: start checking previously completed orders
        if (order.sellPrice < buyOpenOrder.price) {
            console.log(`[...] sell price is lower than buy: ${order.sellPrice} < ${buyOpenOrder.price}`);
            return false;
        }
