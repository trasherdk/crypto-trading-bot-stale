...
class Trader {
    ...
    shouldBuy = (order) => {
        ...
        if (order.buyPrice > sellOpenOrder.price) {
            console.log(`[...] skipping, buy price is higher than sell: ${order.buyPrice} > ${sellOpenOrder.price}`);
            return false;
        }
    ...
    shouldSell = (order) => {
        ...
        if (order.sellPrice < buyOpenOrder.price) {
            console.log(`[...] skipping, sell price is lower than buy: ${order.sellPrice} < ${buyOpenOrder.price}`);
            return false;
        }
