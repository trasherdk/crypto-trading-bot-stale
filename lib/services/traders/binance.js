...
/** percentage of how much order price can be different (lower|higher) that current asset pricer */
...
export default class BinanceTrader {
    ...
    shouldBuy = (order) => {
        ...
        if (buyOpenOrder !== null) {
            ...
            if (this.closedOrders.length > 0) {
                let lastClosedOrder = this.getLastClosedOrder('sell');
                if (lastClosedOrder !== null) {
                    ...
                    const delta3 = Math.abs((this.price.bid - order.buyPrice) / order.buyPrice);

                    const spread = this.getBuySpread() * DELTA_SPREAD_COEFFICIENT;
                    // console.log(`[${ this.market }] [?] shouldBuy() spread: ${ spread }, delta1: ${ delta1 }, delta2: ${ delta2 }`, order.buyPrice, lastClosedOrder.price, buyOpenOrder.price);
                    if (delta1 > spread && delta2 > spread && delta3 > MAX_PRICE_FLUCTUATION) {
    ...
    shouldSell = (order) => {
        ...
        if (sellOpenOrder !== null) {
            ...
            if (this.closedOrders.length > 0) {
                let lastClosedOrder = this.getLastClosedOrder('buy');
                if (lastClosedOrder !== null) {
                    ...
                    const delta3 = Math.abs((this.price.ask - order.sellPrice) / order.sellPrice);

                    const spread = this.getSellSpread() * DELTA_SPREAD_COEFFICIENT;
                    // console.log(`[${ this.market }] [?] shouldSell() spread: ${ spread }, delta1: ${ delta1 }, delta2: ${ delta2 }`, order.sellPrice, lastClosedOrder.price, sellOpenOrder.price);
                    if (delta1 > spread && delta2 > spread && delta3 < MAX_PRICE_FLUCTUATION) {
